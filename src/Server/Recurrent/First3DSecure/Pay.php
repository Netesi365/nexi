...
class Pay extends Nexirequest
{
	...
	public function action(string $codTrans = '', string $importo = '', string $divisa = '', string $xpayNonce = '', string $contractNumber = '') : string {
		try {
			if (empty($codTrans) || empty($importo) || empty($divisa) || empty($xpayNonce) || empty($contractNumber)) {
				throw new \Exception('Missing a parameter',002);
			}
			$timeStamp = (time()) * 1000;
			$mac = sha1('apiKey=' . $this->apiKey . 'numeroContratto=' . $contractNumber . "codiceTransazione=" . $codTrans . "importo=" . $importo . 'divisa=' . $divisa . "xpayNonce=" . $xpayNonce . "timeStamp=" . $timeStamp . $this->secret);
			...
			$guzrequest = $client->request('POST', '/ecomm/api/recurring/primoPagamento3DS', [
				'connect_timeout' => 1.5,
				'headers' => [
