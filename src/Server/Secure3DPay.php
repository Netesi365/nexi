...
{
	public function action(string $codTrans = '', string $importo = '', string $divisa = '', string $params) : string {
		try {
			...
			if (empty($codTrans) || empty($importo) || empty($divisa) || empty($request['esito']) || empty($request['messaggio']) || empty($request['idOperazione']) || empty($request['xpayNonce']) || empty($request['timeStamp']) || empty($request['mac'])) {
			...
			$timeStamp = (time()) * 1000;
			$mac = sha1('apiKey=' . $this->apiKey . 'codiceTransazione=' . $codTrans  . 'importo=' . $importo .  'divisa=' . $divisa . 'xpayNonce=' . $request['xpayNonce'] . 'timeStamp=' . $timeStamp . $this->secret);
